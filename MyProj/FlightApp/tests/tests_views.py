...
class GenerateReportTest(TestCase):
    ...
    def setUpTestData(cls):
      ...
      voo = Voo.objects.create(codigo_voo = 'TAM0001',
						companhia_aerea = "TAM",
						estado = estado_voo,
						rota = rota_voo,
						previsao = previsao_voo,
						real = real_voo)
      Relatorio.objects.create(id_relatorio = 1, codigo_voo = voo.codigo_voo, tipo = 'voo', 
						tempo_real_atraso = real_voo.chegada_real - previsao_voo.chegada_prevista, 
						data_geracao = datetime.datetime(2022, 11, 4, 15, 34, 10, tzinfo=datetime.timezone.utc)
						)
		

    def test_view_url_exists_at_desired_location(self):
        response = self.client.get('/GerarRelatorios/')
        self.assertEqual(response.status_code, 200)

    def test_view_url_accessible_by_name(self):
        response = self.client.get(reverse('GerarRelatorios'))
        self.assertEqual(response.status_code, 200)

    def test_view_uses_correct_template(self):
        response = self.client.get(reverse('GerarRelatorios'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'template/GerarRelatorios.html')
